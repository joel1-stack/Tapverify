...
class PaymentReportService {
  ...
  static Future<String> savePdf({
    ...
    final file = File(
        '${dir.path}/tapverify_${reportTitle.replaceAll(RegExp(r'[^A-Za-z0-9]+'), '_')}_$stamp.pdf');
  ...
  static Future<void> share({
    ...
    await Share.shareXFiles(
      ...
      subject: 'Paid members report — $reportTitle',
      text:
  ...
  static Future<String> saveOutstandingPdf({
    ...
    final stamp = DateTime.now().millisecondsSinceEpoch;
    final file = File(
        '${dir.path}/tapverify_unpaid_${reportTitle.replaceAll(RegExp(r'[^A-Za-z0-9]+'), '_')}_$stamp.pdf');
    await file.writeAsBytes(bytes);
    return file.path;
  }
  ...
  static Future<void> shareOutstanding({
    ...
    final path = await saveOutstandingPdf(
      orgName: orgName,
      reportTitle: reportTitle,
      members: members,
    );
    await Share.shareXFiles(
      [XFile(path, mimeType: 'application/pdf')],
      subject: 'Unpaid members — $reportTitle',
      text:
  ...
  static Future<String> saveRegisterPdf({
    ...
    final stamp = DateTime.now().millisecondsSinceEpoch;
    final file = File(
        '${dir.path}/tapverify_register_${orgName.replaceAll(RegExp(r'[^A-Za-z0-9]+'), '_')}_$stamp.pdf');
    await file.writeAsBytes(bytes);
    return file.path;
  }
  ...
  static Future<void> shareRegister({
    ...
    final path = await saveRegisterPdf(
      orgName: orgName,
      orgType: orgType,
      contributions: contributions,
    );
    await Share.shareXFiles(
      [XFile(path, mimeType: 'application/pdf')],
      subject: 'Activity register — $orgName',
      text:
